...
// create 1000 posts -> we should get the dimension bonus on top
// 3 dimension -> 1.5 times the values
for ($i = 0; $i < 1000; $i++) {
  $post = new Post();
  ...
  // now create a second dimension
  $attention_dimension_entry = new \cls\data\AttentionDimensionEntry();
  $attention_dimension_entry->post_id = $post->id;
  $attention_dimension_entry->attention_dimension_id = 2;
  // we want for test purposes a different order
  $attention_dimension_entry->position_points = $i+1;
  $attention_dimension_entry->number_of_ratings = 50; // eac post has 50 ratings
  $attention_dimension_entry->save(App::get_connection());

  // now create a third dimension
  $attention_dimension_entry = new \cls\data\AttentionDimensionEntry();
  $attention_dimension_entry->post_id = $post->id;
  $attention_dimension_entry->attention_dimension_id = 3;
  // we want for test purposes a different order
  $attention_dimension_entry->position_points = $i+1;
  $attention_dimension_entry->number_of_ratings = 50; // eac post has 50 ratings
  $attention_dimension_entry->save(App::get_connection());
...
assert($results == [
    ...
    300,300,300,300,300,300,300,300,300,300,
    300,300,300,300,300,300,300,300,300,300,
    300,300,300,300,300,300,300,300,300,750,
    ...
    4850,4850,4850,4850,4850,4850,4850,4850,4850,4850,
    4850,4850,4850,4850,4850,4850,4850,4850,4850,4850,
    10112,10112,10112,10112,10112,10112,10112,10112,10112,10112,
