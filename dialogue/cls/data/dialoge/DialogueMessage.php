...
class  DialogueMessage extends DataClass {
   ...
   * @throws Exception
   */
  function get_content_where_liked_stuff_is_underlined(App $app): string {
    # todo: this only works for two user conversations
    # todo: this only works if like sections are not overlapping
    $content = $app->markdown_to_html($this->content);
  ...
  function get_view_card(
    ...
    $membership = $dialogue->get_membership_of_given_account($app, $app->get_currently_logged_in_account()->id);
    ...
    <div class="w3-card-4 w3-margin w3-padding" style="<?= $style ?>">
      ...
      ><?php echo $this->get_content_where_liked_stuff_is_underlined($app)?></div>
      ...
      >
        ...
        <div><!-- comment form-info-box-1 -->
          ...
          your <?= $membership->get_absolute_amount_of_FREE_like_credits($app) ?> free like points)
