...
function publish_blueprint(
  ...
  try {
    ...
    if (
      $blueprint->user_is_allowed_to_publish_blueprint(
        $app,
        $app->get_currently_logged_in_account()->id
      ) === false
    ) {
      return new RequestError(
        dev_message: "You are not allowed to publish this blueprint.",
        code: RequestError::RULE_ERROR,
      );
    }

    if ($blueprint->published === 1) {
      return new RequestError(
        dev_message: "This blueprint is already published.",
        code: RequestError::RULE_ERROR,
      );
    }

    $blueprint->published = 1;
